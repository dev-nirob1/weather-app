...
function App() {
  ...
  const fetchCitys = async (query) => {
    if (query) {
      const res = await axios.get(`http://api.openweathermap.org/geo/1.0/direct?q=${query}&limit=10&appid=${import.meta.env.VITE_apiKeys}`)
    ...
    <div className='w-full min-h-screen bg-gradient-to-br from-[#012534] to-[#51143D] text-white font-semibold'>
      <div className="container mx-auto h-full py-4 px-3 md:px-0">
        ...
        <div className='max-w-xl mx-auto h-full text-neutral-900 relative'>
          ...
          {
            city === '' && <p className='bg-white/20 p-2 text-center rounded flex items-center justify-center'> Please search your location by city name</p>
          }
